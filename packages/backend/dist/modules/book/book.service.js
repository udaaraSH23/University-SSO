 ...
 */
export class BookReader extends BaseService {
    constructor() {
    ...
    async getBookDetails(bookId) {
        ...
        if (isNaN(id)) {
            throw new DomainError("Invalid book ID format", ERROR_CODES.VALIDATION_ERROR, 400);
        }
        try {
            const book = await bookRepository.findBookById(id);
            if (!book) {
                throw new DomainError("Book not found", ERROR_CODES.BOOK_NOT_FOUND, 404);
            }
            return {
                id: book.id,
                title: book.title,
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get book details", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    /**
    ...
    async searchBooks(query, page = 1, limit = 10) {
        ...
        try {
            const { books, total } = await bookRepository.searchBooks(query, page, limit);
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to search books", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
}
/**
 ...
 */
export class BookManager extends BookReader {
    constructor() {
        super();
    }
    /**
    ...
    async createBook(data) {
        ...
        try {
            ...
            const result = await prisma.book.create({
                data: Object.assign(Object.assign({}, data), { available_copies: (_a = data.available_copies) !== null && _a !== void 0 ? _a : data.total_copies }),
            });
            return result;
        }
        catch (err) {
            ...
            if (err.code === "P2002") {
                throw new DomainError("Book with this ISBN already exists", ERROR_CODES.VALIDATION_ERROR, 409);
            }
            throw new RepositoryError("Failed to create book", ERROR_CODES.DB_FAILURE);
        }
    }
    /**
    ...
    async updateBook(id, data) {
        ...
        try {
            ...
            if (err.code === "P2025") {
                // Record not found
                throw new DomainError("Book not found for update", ERROR_CODES.BOOK_NOT_FOUND, 404);
            }
            throw new RepositoryError("Failed to update book", ERROR_CODES.DB_FAILURE);
        }
    }
    /**
    ...
    async deleteBook(id) {
        ...
        try {
            ...
            if (err.code === "P2025") {
                throw new DomainError("Book not found for deletion", ERROR_CODES.BOOK_NOT_FOUND, 404);
            }
            throw new RepositoryError("Failed to delete book", ERROR_CODES.DB_FAILURE);
        }
    }
    /**
    ...
    async getBookById(id) {
        ...
        try {
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get book by id", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
}
