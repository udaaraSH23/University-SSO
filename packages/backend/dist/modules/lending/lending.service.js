...
export class LendingService extends BaseService {
    ...
    async issueBook(studentId, bookId) {
        ...
        try {
            ...
            if (!book || book.available_copies < 1) {
                throw new DomainError("Book not available", ERROR_CODES.LENDING_BOOK_UNAVAILABLE, 400);
            }
            const studentProfile = await prisma.studentProfile.findUnique({
                where: { student_id: studentId },
            });
            if (!studentProfile) {
                throw new DomainError("Student not found", ERROR_CODES.STUDENT_NOT_FOUND, 404);
            }
            if (!studentProfile.isLibraryRegistered) {
                throw new DomainError("Student is not registered for library facilities", ERROR_CODES.LENDING_STUDENT_NOT_REGISTERED, 403);
            }
    ...
    async returnBookByDetails(studentId, isbn) {
        ...
        try {
            ...
            if (!book) {
                throw new DomainError("Book not found with this ISBN", ERROR_CODES.BOOK_NOT_FOUND, 404);
            }
            ...
            if (!record) {
                throw new DomainError("No active borrow record found for this student and book", ERROR_CODES.LENDING_RECORD_NOT_FOUND, 404);
            }
            ...
            if (err instanceof DomainError)
                throw err;
            throw new RepositoryError("Failed to return book by details", ERROR_CODES.DB_FAILURE);
        }
    }
    /**
    ...
    async returnBook(recordId) {
        ...
        try {
            ...
            if (!record)
                throw new DomainError("Record not found", ERROR_CODES.LENDING_RECORD_NOT_FOUND, 404);
            if (record.status === "RETURNED")
                throw new DomainError("Book already returned", ERROR_CODES.LENDING_ALREADY_RETURNED, 400);
            const result = await prisma.$transaction(async (tx) => {
                await tx.book.update({
            ...
            if (err instanceof DomainError)
                throw err;
            throw new RepositoryError("Failed to return book", ERROR_CODES.DB_FAILURE);
        }
    }
    async getPendingBooks() {
        ...
        try {
            ...
            throw new RepositoryError("Failed to get pending books", ERROR_CODES.DB_FAILURE);
        }
    }
    async getLibraryDashboardStats() {
        this.logger.debug({}, "getLibraryDashboardStats called");
        try {
            const [availableBooksAgg, totalBooksAgg, borrowedBooksCount, overdueBooksCount, uniqueBooksCount, totalStudentsCount,] = await Promise.all([
            ...
            throw new RepositoryError("Failed to get library stats", ERROR_CODES.DB_FAILURE);
        }
    }
    async checkBookAvailability(query) {
        ...
        try {
            ...
            throw new RepositoryError("Failed to check book availability", ERROR_CODES.DB_FAILURE);
        }
    }
    async getStudentLibraryStats() {
        ...
        try {
            ...
            throw new RepositoryError("Failed to get student library stats", ERROR_CODES.DB_FAILURE);
        }
    }
    async searchStudentsForLibrary(query) {
        ...
        try {
            ...
            throw new RepositoryError("Failed to search students for library", ERROR_CODES.DB_FAILURE);
        }
    }
    async getStudentBorrowHistory(studentId, page, limit) {
        this.logger.debug({ studentId, page, limit }, "getStudentBorrowHistory called");
        try {
            const student = await prisma.studentProfile.findUnique({
                where: { student_id: studentId },
            });
            ...
            if (err instanceof DomainError)
                throw err;
            throw new RepositoryError("Failed to get student borrow history", ERROR_CODES.DB_FAILURE);
        }
    }
    async toggleStudentRegistration(studentId) {
        ...
        try {
            ...
            if (err instanceof DomainError)
                throw err;
            throw new RepositoryError("Failed to toggle registration", ERROR_CODES.DB_FAILURE);
        }
    }
    async getBookBorrowHistory(bookId) {
        ...
        try {
            ...
            throw new RepositoryError("Failed to get book borrow history", ERROR_CODES.DB_FAILURE);
        }
    }
}
