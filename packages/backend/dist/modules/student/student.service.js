...
export class StudentService extends BaseService {
    ...
    async getProfile(email) {
        ...
        try {
            ...
            const result = {
                ...
                degreeProgramId: profile.degreeProgramId || 0,
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get profile", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    async getCourses(email, filters) {
        ...
        try {
            ...
            const enrollments = await this.studentRepository.findEnrollments(profile.id, filters);
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get courses", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    async getGrades(email) {
        ...
        try {
            ...
            const enrollments = await this.studentRepository.findEnrollments(profile.id);
            const result = enrollments.map((enrollment) => ({
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get grades", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    async getBorrowedBooks(email) {
        ...
        try {
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get borrowed books", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    async getPaginatedStudents(filters) {
        this.logger.debug({ filters }, "Fetching paginated students");
        try {
            const page = filters.page || 1;
            const limit = filters.limit || 10;
            const query = filters.query || filters.search;
            const { total, students } = await this.studentRepository.findPaginatedStudents(Object.assign(Object.assign({}, filters), { query,
                page,
                limit }));
            const studentDTOs = students.map((student) => {
                ...
                return ({
                    ...
                    gpa: student.gpa || 0,
                    degreeProgram: ((_a = student.degreeProgram) === null || _a === void 0 ? void 0 : _a.name) || "N/A",
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get paginated students", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    async getStudentDetailById(id) {
        ...
        try {
            ...
            const enrollments = student.enrollments.map((enr) => ({
                id: enr.id,
                enrollmentId: enr.id,
            ...
            return {
                profile: {
                    ...
                    degreeProgramId: student.degreeProgramId || 0,
                    currentAcademicYear: student.currentAcademicYear,
                    level: student.level,
                    isLibraryRegistered: student.isLibraryRegistered,
                },
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to get student details", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    async createStudent(data) {
        ...
        try {
            ...
            try {
                ...
                this.logger.warn({ err }, "Failed to generate invite link, but student created");
            }
            // 4. Save to database
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to create student", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
        ...
        try {
            ...
            if (data.fullName && data.fullName !== current.full_name) {
                try {
                    ...
                    this.logger.warn({ err }, "Failed to sync name change to Identity Server");
                }
            }
            return {
                id: updated.id,
                ...
                gpa: updated.gpa || 0,
                degreeProgram: ((_a = updated.degreeProgram) === null || _a === void 0 ? void 0 : _a.name) || "N/A",
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to update student", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
    async deleteStudent(id) {
        ...
        try {
            ...
            try {
                ...
                this.logger.error({ error, wso2_id: student.user.wso2_id }, "Failed to delete user from WSO2, proceeding with local delete");
            }
            ...
            if (err instanceof RepositoryError)
                throw err;
            throw new DomainError("Failed to delete student", ERROR_CODES.INTERNAL_ERROR, 500);
        }
    }
}
