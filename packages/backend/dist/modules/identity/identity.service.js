...
export class IdentityService {
    constructor() {
        this.baseUrl =
            process.env.WSO2_BASE_URL || "https://wso2is.com/t/universityportal.com";
        ...
        try {
            const response = await fetch(this.tokenUrl, {
                ...
                body: new URLSearchParams({
                    ...
                    scope: "internal_offline_invite internal_group_mgt_view internal_group_mgt_update internal_user_mgt_create internal_user_mgt_delete internal_user_mgt_update internal_user_mgt_list internal_user_mgt_view",
                }),
            });
            if (!response.ok) {
                const error = await response.text();
                logger.error({ error }, "Failed to fetch WSO2 token");
                throw new DomainError("Failed to authenticate with Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR, 500);
            }
            ...
            throw new RepositoryError("Failed to connect to Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR);
        }
    }
    /**
    ...
    async createUser(data) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new DomainError("Failed to create user in Identity Server", ERROR_CODES.IDENTITY_CREATION_FAILED, response.status);
            }
            ...
            throw new RepositoryError("Failed to create user in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR);
        }
    }
    /**
     ...
     */
    async addUserToGroup(userId, groupName, username) {
        var _a, _b;
        logger.debug({ userId, groupName }, "Adding user to group in WSO2");
        try {
            const token = await this.getManagementToken();
            // First find the group ID
            const groupResponse = await fetch(`${this.baseUrl}/scim2/Groups?filter=displayName eq ${groupName}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            const groups = await groupResponse.json();
            logger.debug({ groups }, "WSO2 Group Data");
            const groupId = (_b = (_a = groups.Resources) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.id;
            if (!groupId) {
                logger.error({ groupName }, "Group not found in WSO2");
                throw new DomainError(`Group ${groupName} not found in Identity Server`, ERROR_CODES.RESOURCE_NOT_FOUND, 404);
            }
            const memberValue = { value: userId };
            if (username) {
            ...
            if (!response.ok) {
                ...
                throw new DomainError("Failed to assign role in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR, response.status);
            }
        }
        catch (error) {
            if (error instanceof DomainError)
            ...
            throw new RepositoryError("Failed to assign group in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR);
        }
    }
    /**
    ...
    async generateInviteLink(email) {
        ...
        try {
            const token = await this.getManagementToken();
            const response = await fetch(`${this.baseUrl}/o/api/users/v1/offline-invite-link/`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new DomainError("Failed to generate invitation in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR, response.status);
            }
            const result = await response.json();
            ...
            throw new RepositoryError("Failed to generate invite link", ERROR_CODES.IDENTITY_SERVER_ERROR);
        }
    }
    /**
    ...
    async updateUser(id, data) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new DomainError("Failed to update user in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR, response.status);
            }
        }
        catch (error) {
            if (error instanceof DomainError)
            ...
            throw new RepositoryError("Failed to update user in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR);
        }
    }
    /**
    ...
    async deleteUser(id) {
        ...
        try {
            ...
            if (!response.ok && response.status !== 404) {
                ...
                throw new DomainError("Failed to delete user in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR, response.status);
            }
        }
        catch (error) {
            if (error instanceof DomainError)
            ...
            throw new RepositoryError("Failed to delete user in Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR);
        }
    }
    /**
    ...
    async getGroups() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new DomainError("Failed to fetch groups", ERROR_CODES.IDENTITY_SERVER_ERROR, response.status);
            }
            const data = await response.json();
            ...
            throw new RepositoryError("Failed to fetch groups from Identity Server", ERROR_CODES.IDENTITY_SERVER_ERROR);
        }
    }
}
